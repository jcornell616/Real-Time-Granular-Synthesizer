...
void InitAIC23(void) {
    ...
    //command = fullpowerup();            //Turn on everything
    SpiTransmit(command);
}
...
void InitMcBSPbDSPMode() {
    EALLOW;
     ...
     McbspbRegs.RCR1.bit.RFRLEN1 = 0; // Receive frame length = 1 word in phase 1
     McbspbRegs.RCR2.bit.RFRLEN2 = 0; // Receive frame length = 1 word in phase 2
     McbspbRegs.XCR2.bit.XPHASE = 0; // Send high 16 bits
     McbspbRegs.XCR1.bit.XFRLEN1 = 0; // Transmit frame length = 1 word in phase 1
     McbspbRegs.XCR2.bit.XFRLEN2 = 0; // Transmit frame length = 1 word in phase 2
...
void InitMcBSPbIntRX(Uint16 function_addr) {
    ...
    PieVectTable.MCBSPB_RX_INT = function_addr;
...
void InitMcBSPbIntTX(Uint16 function_addr) {
    DINT;
    EALLOW;
    PieCtrlRegs.PIECTRL.bit.ENPIE = 1;
    PieVectTable.MCBSPB_TX_INT = function_addr;
...
void SampleRate_8In_48Out(void) {
        SpiTransmit(CLKsampleratecontrol(0x8));
        SmallDelay();
}

void SampleRate_48In_8Out(void) {
    SpiTransmit(CLKsampleratecontrol(0x4));
    SmallDelay();
}
...
void SampleRate_32InOut(void) {
    SpiTransmit(softpowerdown());
    SmallDelay();
    SpiTransmit(CLKsampleratecontrol(SR32));
    SmallDelay();
    SpiTransmit(nomicpowerup());
    SmallDelay();
}

void SampleRate_8InOut(void) {
    SpiTransmit(CLKsampleratecontrol(0xC));
    SmallDelay();
}
